...
public final class Kamaloka extends Quest
{
	...
	//			player.sendPacket(sm); //TODO:
			...
			{
				 //TODO:	SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.C1_IS_IN_A_LOCATION_WHICH_CANNOT_BE_ENTERED_THEREFORE_IT_CANNOT_BE_PROCESSED);
			//	sm.addPcName(partyMember);
	...
	private synchronized void enterInstance(Player player, int index)
	{
		...
		if (world != null)
		{
			...
			if (!(world instanceof KamaWorld) || (world.getTemplateId() != templateId))
			{
				...
				player.sendMessage("You have entered another instant zone therefore you cannot enter corresponding dungeon");
				return;
			}
