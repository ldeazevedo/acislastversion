...
import net.sf.l2j.gameserver.model.actor.instance.WyvernManagerNpc;
...
public abstract class ASpawn
{
	...
	public Npc doSpawn(boolean isSummonSpawn)
	{
		try
		{
			// Check if specific NPCs can be spawned.
			final Class<?> clazz = _constructor.getDeclaringClass();
			if (!Config.ALLOW_CLASS_MASTERS && clazz == ClassMaster.class)
			{
				LOGGER.warn("ClassMaster (id {}) spawn is disabled by config.", getNpcId());
				return null;
			}
			
			if (!Config.WYVERN_ALLOW_UPGRADER && clazz == WyvernManagerNpc.class)
			{
				LOGGER.warn("WyvernManagerNpc (id {}) spawn is disabled by config.", getNpcId());
				return null;
			}
